...
export const sendTaskAssignmentEmail = async (email, name, taskDetails) => {
  const { title, description, priority, dueDate } = taskDetails;
  const content = `
    <h2 style="color: #ffffff; font-size: 24px; font-weight: 800; margin: 0 0 20px 0;">Mission <span style="color: #F05E23;">Assigned</span></h2>
    <p>Greetings <b>${name}</b>, a new tactical objective has been assigned to your workspace. Please review the parameters and begin initialization.</p>
    
    <div style="margin: 40px 0; padding: 35px; background: rgba(240, 94, 35, 0.03); border: 1px solid rgba(240, 94, 35, 0.1); border-radius: 30px;">
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td style="padding-bottom: 25px;">
            <p style="margin: 0; font-size: 10px; font-weight: 800; color: #F05E23; text-transform: uppercase; letter-spacing: 2px;">Directive Parameters</p>
          </td>
        </tr>
        <tr>
          <td style="padding-bottom: 20px;">
            <span style="display: block; font-size: 12px; color: rgba(255,255,255,0.4); margin-bottom: 6px;">Objective Title</span>
            <span style="display: block; font-size: 18px; font-weight: 700; color: #ffffff;">${title}</span>
          </td>
        </tr>
        <tr>
          <td style="padding-bottom: 20px;">
            <span style="display: block; font-size: 12px; color: rgba(255,255,255,0.4); margin-bottom: 6px;">Priority Status</span>
            <span style="display: inline-block; padding: 4px 12px; background: ${priority === 'High' ? 'rgba(239, 68, 68, 0.1)' : 'rgba(255, 255, 255, 0.05)'}; border: 1px solid ${priority === 'High' ? '#EF4444' : 'rgba(255, 255, 255, 0.1)'}; border-radius: 8px; color: ${priority === 'High' ? '#EF4444' : '#ffffff'}; font-size: 10px; font-weight: 800; text-transform: uppercase;">${priority}</span>
          </td>
        </tr>
        ${dueDate ? `
        <tr>
          <td style="padding-bottom: 20px;">
            <span style="display: block; font-size: 12px; color: rgba(255,255,255,0.4); margin-bottom: 6px;">Operational Deadline</span>
            <span style="display: block; font-size: 14px; font-weight: 700; color: #ffffff;">${new Date(dueDate).toDateString()}</span>
          </td>
        </tr>
        ` : ''}
        <tr>
          <td>
            <span style="display: block; font-size: 12px; color: rgba(255,255,255,0.4); margin-bottom: 6px;">Mission briefing</span>
            <div style="font-size: 14px; color: rgba(255,255,255,0.8); line-height: 1.6; background: rgba(0,0,0,0.2); padding: 20px; border-radius: 15px;">
              ${description}
            </div>
          </td>
        </tr>
    ...
    <a href="${BASE_URL}/intern" style="display: block; background-color: #F05E23; color: #ffffff; text-align: center; padding: 25px; border-radius: 20px; text-decoration: none; font-weight: 900; font-size: 14px; text-transform: uppercase; letter-spacing: 4px; box-shadow: 0 20px 40px rgba(240,94,35,0.2);">Acknowledge Mission</a>
  `;

  return transporter.sendMail({
    from: `"Sync Command" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: `TACTICAL ASSIGNMENT: ${title}`,
    html: ADVANCED_WRAPPER(content, "#F05E23", "MISSION-ASSIGNED"),
  });
};

export const sendMeetingRequestEmail = async (adminEmails, internName, taskTitle, meetingLink) => {
  const content = `
    <h2 style="color: #ffffff; font-size: 24px; font-weight: 800; margin: 0 0 20px 0;">Meeting <span style="color: #F05E23;">Requested</span></h2>
    <p>Personnel <b>${internName}</b> has requested an immediate synchronization meeting regarding the mission: <b>${taskTitle}</b>.</p>
    
    <div style="margin: 40px 0; padding: 35px; background: rgba(240, 94, 35, 0.03); border: 1px solid rgba(240, 94, 35, 0.1); border-radius: 30px; text-align: center;">
      <p style="margin-bottom: 25px; font-size: 10px; font-weight: 800; color: #F05E23; text-transform: uppercase; letter-spacing: 2px;">Synchronous Bridge Generated</p>
      <a href="${meetingLink}" style="display: block; color: #ffffff; font-size: 18px; font-weight: 700; text-decoration: underline; margin-bottom: 10px;">${meetingLink}</a>
      <p style="font-size: 11px; color: rgba(255,255,255,0.4);">Protocol: Encrypted Direct Link</p>
    </div>

    <div style="margin-bottom: 45px;">
      <p style="color: #ffffff; font-weight: 800; text-transform: uppercase; font-size: 11px; letter-spacing: 1.5px; margin-bottom: 16px;">Next Steps:</p>
      <div style="color: rgba(255,255,255,0.6); font-size: 14px;">
        • Join the generated Google Meet session<br>
        • Resolve blocker details with the assigned intern<br>
        • Update task parameters after synchronization
      </div>
    </div>

    <a href="${meetingLink}" target="_blank" style="display: block; background-color: #F05E23; color: #ffffff; text-align: center; padding: 25px; border-radius: 20px; text-decoration: none; font-weight: 900; font-size: 14px; text-transform: uppercase; letter-spacing: 4px; box-shadow: 0 20px 40px rgba(240,94,35,0.2);">Connect to Sync-Link</a>
  `;

  return transporter.sendMail({
    from: `"Sync Alert" <${process.env.EMAIL_USER}>`,
    to: adminEmails,
    subject: `MEETING REQUEST: ${internName} - ${taskTitle}`,
    html: ADVANCED_WRAPPER(content, "#F05E23", "MEETING-OVERRIDE"),
  });
};
